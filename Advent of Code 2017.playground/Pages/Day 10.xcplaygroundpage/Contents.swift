 ...
 */
let exampleList: [UInt8] = [0, 1, 2, 3, 4]
let exampleLengths = [3, 4, 1, 5]
var skip = 0
var index = exampleList.startIndex
let exampleAnswer = knotHashRound(values: exampleList, lengths: exampleLengths, index: &index, skip: &skip)
assert(exampleAnswer == [3, 4, 2, 1, 0])

let input = loadInput()
let lengths = input.components(separatedBy: ",").flatMap(Int.init)
let values: [UInt8] = Array(0...255)
skip = 0
index = values.startIndex
let result = knotHashRound(values: values, lengths: lengths, index: &index, skip: &skip)

let answer1 = Int(result[0]) * Int(result[1])
 ...
 */
let a: String = knotHash("")
assert(a == "a2582a3a0e66e6e86e3812dcb672a272")

let b: String = knotHash("AoC 2017")
assert(b == "33efeb34ea91902bb2f59c9920caa6cd")

let c: String = knotHash("1,2,3")
assert(c == "3efbe78a8d82f29979031a4aa0b16a9d")

let d: String = knotHash("1,2,4")
assert(d == "63960835bcdc130f0b66d7ff4f6a5a8e")
